...
function CardsList(props) {
 ...
 // You can put this in the dependency to
  ...
  const fetchCards = (boardId) => { axios
    .get(
  ...
  const increaseLikes = (cardId) => {
    setLikes(likes + 1);
      axios.put(`${process.env.REACT_APP_BACKEND_URL}/cards/${cardId}/likes`, {
          likes_count: likes + 1 
      })
      .then(response => {
        console.log(response.data)
        const newCards = cards.map(oldCard => {
          return oldCard.id !== cardId ? oldCard : {...oldCard, likes_count : oldCard.likes_count + 1} 
        }) 
        setCards (newCards)
      })
      .catch(error => {
